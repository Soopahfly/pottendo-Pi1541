...
/**
    @brief Add a property tag to the current tag list. Data can be included. All
   data is uint32_t
    @param tag
*/
void RPI_PropertyAddTag(rpi_mailbox_tag_t tag, ...) {
  ...
  case TAG_GET_THROTTLED:
    pt[pt_index++] = 8;
    pt[pt_index++] = 0; /* Request */
    pt[pt_index++] = 0; /* Write 0 to clear sticky bits */
    pt_index += 1;
    break;
  ...
  case TAG_SET_PALETTE:
    ...
    // Set the PWR led signal to output mode, allowing to control it : rpi3 -
    // gpiovirtbuf c 135 1 0 0 0 0 Set the PWR led off : rpi3 - gpiovirtbuf s
    // 135 0
  ...
  /* Make sure the tags are 0 terminated to end the list and update the buffer
   * size */
  pt[pt_index] = 0;
