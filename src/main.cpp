...
#define CV
#endif
// When the emulated CPU starts we execute the first million odd cycles in
// non-real-time (ie as fast as possible so the emulated 1541 becomes responsive
// to CBM-Browser asap) During these cycles the CPU is executing the ROM self
// test routines (these do not need to be cycle accurate)
...
// Raspberry Pi throttle status bits (TAG_GET_THROTTLED)
#define THROTTLED_UNDER_VOLTAGE_NOW (1 << 0) // Under-voltage currently detected
#define THROTTLED_FREQ_CAPPED_NOW (1 << 1)   // ARM frequency capped currently
#define THROTTLED_NOW (1 << 2)               // Currently throttled
#define THROTTLED_UNDER_VOLTAGE_OCCURRED                                       \
  (1 << 16) // Under-voltage has occurred since boot

// To exit a mounted disk image we need to watch(snoop) what the emulated CPU is
// doing when it executes code at some critical ROM addresses.
...
int GetThrottled(unsigned &value) {
  rpi_mailbox_property_t *mp;

  RPI_PropertyInit();
  RPI_PropertyAddTag(TAG_GET_THROTTLED);
  RPI_PropertyProcess();

  value = 0;
  if ((mp = RPI_PropertyGet(TAG_GET_THROTTLED))) {
    value = mp->data.buffer_32[1];
    return 1;
  }
...
void MsDelay(unsigned nMilliSeconds) { usDelay(nMilliSeconds * 1000); }
...
void CancelKernelTimer(unsigned hTimer) { TimerCancelKernelTimer(hTimer); }

typedef void TInterruptHandler(void *pParam);

// USPi uses USB IRQ 9
void ConnectInterrupt(unsigned nIRQ, TInterruptHandler *pHandler,
                      void *pParam) {
...
DWORD get_fattime() {
  return 0;
} // If you have hardware RTC return a correct value here. THis can then be
  // reflected in file modification times/dates.
#endif
...
void InitialiseLCD() {
  ...
  if (i2cLcdModel) {
    ...
    screenLCD->Open(width, height, 1, i2cBusMaster, i2cLcdAddress, i2cLcdFlip,
                    i2cLcdModel, i2cLcdUseCBMChar);
    ...
    if ((height >= 64) &&
      ...
      logo_done = true;
               ...
               (strcasecmp(options.GetLcdLogoName(), "1541classic") == 0)) {
      screenLCD->PlotRawImage(logo_ssd_1541classic, 0, 0, width, 64);
      logo_done = true;
    } else if (f_stat(options.GetLcdLogoName(), &filLcdIcon) == FR_OK &&
               filLcdIcon.fsize <= LCD_LOGO_MAX_SIZE) {
      FIL fp;
      FRESULT res;
...
// void UpdateUartControls(bool refreshStatusDisplay, bool LED, bool Motor, bool
// ATN, bool DATA, bool CLOCK, u32 Track, u32 romIndex)
//{
...
//		printf("\E[1ALED %s%d\E[0m Motor %d Track %0d.%d ATN %d DAT %d
// CLK %d %s\r\n", LED ? termainalTextRed : termainalTextNormal, LED, Motor,
// Track >> 1, Track & 1 ? 5 : 0, ATN, DATA, CLOCK, roms.ROMNames[romIndex]);
// }
...
void UpdateLCD(const char *track, unsigned temperature) {
  if (screenLCD) {
    ...
    screenLCD->PrintText(false, 0, 0, tempBuffer, 0,
...
// Care must be taken not to crowd out the shared cache with core1 as this could
// slow down core1 so that it no longer can perform its duties in the 1us
// timings it requires.
void UpdateScreen() {
#if not defined(EXPERIMENTALZERO)
  bool oldLED = false;
  ...
  while (1) {
    ...
    if (options.HDMIDisplayIECActivity()) {
      ...
      if (value != oldLED) {
        ...
        screen->PrintText(false, 4 * 8, y, tempBuffer,
                          value ? COLOUR_RED : textColour, bgColour);
    ...
    if (options.HDMIGraphIEC()) {
      ...
      if (value ^ oldATN) {
        ...
        if (value)
          screen->PlotPixel(graphX, top3, atnColour);
        else
    ...
    if (options.HDMIGraphIEC()) {
      ...
      if (value ^ oldDATA) {
        ...
        if (value)
          screen->PlotPixel(graphX, top2, dataColour);
        else
    ...
    if (options.HDMIGraphIEC()) {
      ...
      if (value ^ oldCLOCK) {
        ...
        if (value)
          screen->PlotPixel(graphX, top, clockColour);
        else
          screen->PlotPixel(graphX, bottom, clockColour);
      }
    }
    if (options.HDMIDisplayIECActivity()) {
      if (value != oldCLOCK) {
    ...
    //	oldSRQ = value;
    ////	snprintf(tempBuffer, tempBufferSize, "%d", value);
    ////	screen->PrintText(false, 41 * 8, y, tempBuffer, textColour,
    /// bgColour); /	//refreshUartStatusDisplay = true;
    //}
    //}

    if (graphX++ > screenWidthM1)
      graphX = 0;
    ...
    if (emulating == EMULATING_1541) {
      ...
      if (track != oldTrack) {
        oldTrack = track;
        snprintf(tempBufferTrack, tempBufferTrackSize, "%02d.%d",
                 (oldTrack >> 1) + 1, oldTrack & 1 ? 5 : 0);
        if (options.HDMIDisplayIECActivity())
          screen->PrintText(false, 20 * 8, y, tempBufferTrack, textColour,
                            bgColour);
        // refreshUartStatusDisplay = true;
      ...
      if (track != oldTrack) {
        oldTrack = track;
        snprintf(tempBufferTrack, tempBufferTrackSize, "%02d  ",
                 (oldTrack) + 1);
        if (options.HDMIDisplayIECActivity())
          screen->PrintText(false, 20 * 8, y, tempBufferTrack, textColour,
                            bgColour);
    ...
    if (emulating != IEC_COMMANDS) {
      ...
      // Putting the semaphore around diskCaddy.Update() keeps this core awake
      // and this breaks emulation on option B hardware. Don't know why.
      // Disabling for now.
      // #if not defined(EXPERIMENTALZERO)
      //			core0RefreshingScreen.Acquire();
      // #endif
      ...
      if (options.DisplayTemperature()) {
        if (GetTemperature(temperature)) {
          ...
          if (temperature != oldTemperature) {
            ...
            screen->PrintText(false, 43 * 8, y, tempBuffer, textColour,
                              bgColour);
            refreshLCDStatusDisplay = true;
          }
        }
    ...
    //	UpdateUartControls(refreshUartStatusDisplay, oldLED, oldMotor, oldATN,
    // oldDATA, oldCLOCK, oldTrack, romIndex);
...
static bool Snoop(u8 a, int max) {
  ...
  if (a == snoopBackCommand[snoopIndex] ||
      (snoopIndex == 2 && (a == snoopBackCommand[3]))) {
    if (snoopIndex == 2 &&
        (a == snoopBackCommand[3])) // The CD_ without the : case
      snoopIndex++;
...
EmulatingMode BeginEmulating(FileBrowser *fileBrowser,
                             const char *filenameForIcon) {
  DiskImage *diskImage = diskCaddy.SelectFirstImage();
  DEBUG_LOG("%s: name = %s, IconName='%s'\n", __FUNCTION__,
            diskImage->GetName(), filenameForIcon);
  if (diskImage) {
#if defined(PI1581SUPPORT)
    if (diskImage->IsD81()) {
...
DMA_ControlBlock dmaSoundCB = {DMA_DEST_DREQ + DMA_PERMAP_5 + DMA_SRC_INC, 0,
                               // 0x7E000000 + PWM_BASE + PWM_FIF1
                               0x7E000000 + 0x20C000 + 0x18,
                               (u32)(Sample_bin_size * 4), 0,
                               0, //&dmaSoundCB,
                               0, 0};
...
void DisplayMessage(int x, int y, bool LCD, const char *message, u32 textColour,
...
EXIT_TYPE __not_in_flash_func(Emulate1541)(FileBrowser *fileBrowser) {
  ...
  // 0x778fecda, 0x6ab92e00 (german version) 0x97732c3e =
  // maniac_mansion_s1[activision_1987](!).g64 0x63f809d2 =
  // 4x4_offroad_racing_s1[epyx_1988](ntsc)(!).g64
  if (hash == 0x42c02586 || hash == 0x18651422 || hash == 0x2a7f4b77 ||
      hash == 0x97732c3e || hash == 0x63f809d2 || hash == 0x778fecda ||
      hash == 0x6ab92e00 || hash == 0x3adb56b7) {
    refreshOutsAfterCPUStep = false;
    DEBUG_LOG("%s: .g64 hash = %x, refreshOutsAfterCPUStep = false",
              __FUNCTION__, hash);
  }
  ...
  while (exitReason == EXIT_UNKNOWN) {
    ...
    if (pi1541.m6502.SYNC()) // About to start a new instruction.
    {
      ...
      // See if the emulated cpu is executing CD:_ (ie back out of emulated
      // image)
      if (pc == SNOOP_CD_CBM || pc == SNOOP_CD_JIFFY_BOTH ||
          pc == SNOOP_CD_JIFFY_BOTH_V6_00 ||
          (pc == SNOOP_CD_JIFFY_DRIVEONLY && (roms.GetHash() != 0x9eef0d97)))
        snoopPC = pc;
    ...
    pi1541.m6502.Step(); // If the CPU reads or writes to the VIA then clk and
                         // data can change

    // To artificialy delay the outputs later into the phi2's cycle (do this on
    // future Pis that will be faster and perhaps too fast)
    ...
    // Other core will check the uart (as it is slow) (could enable uart irqs -
    // will they execute on this core?)
#if not defined(EXPERIMENTALZERO)
    inputMappings->CheckKeyboardEmulationMode(numberOfImages,
                                              numberOfImagesMax);
#endif
...
#endif
    if ((emulating == IEC_COMMANDS) || (resetCount > 10) || exitEmulation ||
        exitDoAutoLoad) {
      if (reset)
    ...
    do // Sync to the 1MHz clock
    {
      ...
      if (ct > 1) {
        // If this ever occurs then we have taken too long (ie >1us) and lost a
        // cycle. Cycle accuracy is now in jeopardy. If this occurs during
        // critical communication loops then emulation can fail!
#if defined(__PICO2__) || defined(ESP32)
        DEBUG_LOG("! ct = %d\n", ct);
#endif
        // delay(1000 * 10);
        // sprintf(tempBuffer, "-%d-", ct);
        // DisplayMessage(0, 20, true, tempBuffer, RGBA(255, 255, 255, 255),
        // RGBA(0,0,0,0));
      }
  ...
  while (exitReason == EXIT_UNKNOWN) {
    ...
    for (int cycle2MHz = 0; cycle2MHz < 2; ++cycle2MHz) {
      if (pi1581.m6502.SYNC()) // About to start a new instruction.
      {
        ...
        // See if the emulated cpu is executing CD:_ (ie back out of emulated
        // image)
        if (pc == SNOOP_CD_CBM1581 || pc == SNOOP_CD_CBM1581_JIFFY)
          snoopPC = pc;
...
void __not_in_flash_func(emulator)(void) {
  ...
  fileBrowser = new FileBrowser(inputMappings, &diskCaddy, &roms, &deviceID,
                                options.DisplayPNGIcons(), screen, screenLCD,
                                options.ScrollHighlightRate());
  pi1541.Initialise();

  _m_IEC_Commands->SetAutoBootFB128(options.AutoBootFB128());
  _m_IEC_Commands->Set128BootSectorName(options.Get128BootSectorName());
  _m_IEC_Commands->SetLowercaseBrowseModeFilenames(
      options.LowercaseBrowseModeFilenames());
  ...
  while (1) {
    if (emulating == IEC_COMMANDS) {
      ...
      inputMappings->Reset();
#if not defined(EXPERIMENTALZERO)
      inputMappings->SetKeyboardBrowseLCDScreen(
      ...
      if (!options.GetDisableSD2IECCommands()) {
        ...
        while (emulating == IEC_COMMANDS) {
          IEC_Commands::UpdateAction updateAction =
              _m_IEC_Commands->SimulateIECUpdate();
          ...
          case IEC_Commands::NONE:
            ...
            if (fileBrowser->SelectionsMade())
              emulating =
                  BeginEmulating(fileBrowser, fileBrowser->LastSelectionName());
            break;
          case IEC_Commands::IMAGE_SELECTED:
            ...
            if (DiskImage::IsLSTExtention(fileBrowserSelectedName)) {
              if (fileBrowser->SelectLST(fileBrowserSelectedName)) {
                emulating =
                    BeginEmulating(fileBrowser, fileBrowserSelectedName);
              } else {
                _m_IEC_Commands->Reset();
                fileBrowserSelectedName = 0;
              }
            } else if (DiskImage::IsDiskImageExtention(
                           fileBrowserSelectedName)) {
              const FILINFO *filInfoSelected =
                  _m_IEC_Commands->GetImageSelected();
          ...
          FILINFO fi;
          if (mount_new) {
            DEBUG_LOG("%s: webserver requests to mount in dir '%s' img '%s'",
                      __FUNCTION__, mount_path, mount_img);
            ...
            if (t) {
              ...
              if ((r = f_chdrive(mount_path)) != FR_OK)
                DEBUG_LOG("%s: f_chdrive to '%s' failed with %d", __FUNCTION__,
                          mount_path, r);
              *t = tt;
            } else {
              DEBUG_LOG("%s: mount_path '%s' has no drive specifier!",
      ...
      if ((exitReason == EXIT_RESET) &&
          (options.GetOnResetChangeToStartingFolder() ||
           selectedViaIECCommands))
...
static bool AttemptToLoadROM(const char *ROMName, int index) {
  FIL fp;
  FRESULT res;
  ...
  if ((FR_OK == f_open(&fp, ROMName, FA_READ)) ||
    ...
    SetACTLed(false);
    f_close(&fp);
    DEBUG_LOG("Opened ROM %s %d %d %d %08x\r\n", ROMName, ROMs::ROM_SIZE,
              bytesRead, bytesRead == ROMs::ROM_SIZE, hash);
    return true;
  } else {
    roms.ROMValid[index] = false;
...
static void DisplayLogo() {
  ...
  stbi_uc *image =
      stbi_load_from_memory((stbi_uc const *)I__logo_png, I__logo_png_size, &w,
                            &h, &channels_in_file, 0);
...
#if !defined(__CIRCLE__) && !defined(__PICO2__) && !defined(ESP32)
static void CheckPowerSupply() {
  unsigned throttled = 0;
  if (GetThrottled(throttled)) {
    if (throttled &
        (THROTTLED_UNDER_VOLTAGE_NOW | THROTTLED_UNDER_VOLTAGE_OCCURRED)) {
#if not defined(EXPERIMENTALZERO)
      const char *msg = "WARNING: Insufficient power supply detected!";
      u32 widthText, heightText;
      screen->MeasureText(false, (char *)msg, &widthText, &heightText);
      u32 xpos = (screen->Width() > widthText)
                     ? (screen->Width() - widthText) >> 1
                     : 0;
      u32 ypos = (screen->Height() > heightText + 4)
                     ? screen->Height() - heightText - 4
                     : 0;
      screen->PrintText(false, xpos, ypos, (char *)msg, COLOUR_WHITE,
                        COLOUR_RED);
#endif
      if (screenLCD) {
        screenLCD->PrintText(false, 0, 0, (char *)"POWER WARNING!",
                             RGBA(0xff, 0xff, 0xff, 0xff));
        screenLCD->RefreshScreen();
      }
    }
...
void DisplayOptions(int y_pos) {
  ...
  snprintf(tempBuffer, tempBufferSize, "ignoreReset = %d\r\n",
           options.IgnoreReset());
  screen->PrintText(false, 0, y_pos += 16, tempBuffer, COLOUR_WHITE,
                    COLOUR_BLACK);
...
static void CheckOptions() {
  ...
  if (FontROMName) {
    ...
    if ((FR_OK == f_open(&fp, FontROMName, FA_READ)) ||
      ...
      xpos = (widthScreen - widthText) >> 1;
      ypos = (heightScreen - heightText) >> 1;
      screen->PrintText(false, xpos, ypos, tempBuffer, COLOUR_WHITE,
                        COLOUR_RED);
  ...
  for (ROMIndex = ROMs::MAX_ROMS - 1; ROMIndex >= 0; --ROMIndex) {
    ...
    AttemptToLoadROM(ROMName, ROMIndex);
  }

  if (roms.ROMValid[0] == false &&
      !(AttemptToLoadROM("d1541.rom", 0) || AttemptToLoadROM("dos1541", 0) ||
        AttemptToLoadROM("d1541II", 0) || AttemptToLoadROM("Jiffy.bin", 0))) {
    snprintf(
        tempBuffer, tempBufferSize,
        "No ROM file found!\r\nPlease copy a valid 1541 ROM file in the root "
...
void UpdateFirmwareToSD() {
  ...
  if (SwitchDrive("USB01:")) {
    DEBUG_LOG("%s: found USB01, checking for firmware update; expected kernel: "
              "%s\r\n",
            ...
            if ((res == FR_OK) && ((u32)filInfo.fsize == bytes)) {
              if (SwitchDrive("SD:")) {
                if (f_chdir("\\") == FR_OK) {
                  ...
                  screen->Clear(COLOUR_BLACK);
                  if (!same && (FR_OK == f_open(&fp, firmwareName,
                                                FA_CREATE_ALWAYS | FA_WRITE))) {
                    snprintf(tempBuffer, tempBufferSize,
                             "Updating firmware.\r\n");
                    screen->MeasureText(false, tempBuffer, &widthText,
                                        &heightText);
                    xpos = (widthScreen - widthText) >> 1;
                    ypos = (heightScreen - heightText) >> 1;
                    screen->PrintText(false, xpos, ypos, tempBuffer,
...
void DisplayMessage(int x, int y, bool LCD, const char *message, u32 textColour,
  ...
  if (!LCD) {
    ...
    screen->PrintText(false, x, y, (char *)message, textColour,
                      backgroundColour);
  } else if (screenLCD) {
    ...
    screenLCD->PrintText(false, x, y, (char *)message, textColour,
                         backgroundColour);
...
void list_directory(const char *path) {
  ...
  while (1) {
    ...
    if (fr != FR_OK || entry.filImage.fname[0] == 0)
  ...
  for (auto it : list.entries) {
    if (it.filImage.fattrib & AM_DIR) {
      ...
      printf("[FILE] %s\t(%lu bytes)\n", it.filImage.fname,
             (unsigned long)it.filImage.fsize);
    }
  }
  printf("%s: done\n", __FUNCTION__);
}
...
void kernel_main(unsigned int r0, unsigned int r1, unsigned int atags) {
  ...
  headSoundFreq =
      1000000 / options.SoundOnGPIOFreq(); // 1200Hz = 1/1200 * 10^6;
  headSoundCounterDuration = 1000 * options.SoundOnGPIODuration();
  DEBUG_LOG("Sound: %s", (playsound > 0)    ? "GPIO"
                         : (playsound == 0) ? "DMA"
                                            : "OFF");
  if (playsound > 0)
    DEBUG_LOG("%d Freq, %dus duration", options.SoundOnGPIOFreq(),
              options.SoundOnGPIODuration());
  ...
  // ROTARY: Added for rotary encoder inversion (Issue#185) - 08/13/2020 by
  // Geo...
  ...
  for (int USBDriveIndex = 0; USBDriveIndex < numberOfUSBMassStorageDevices;
       ++USBDriveIndex) {
    char USBDriveId[16];
  ...
  if (options.GetHeadLess() && options.GetDisableHDMI()) {
    ...
    UpdateScreen(); // core0 now loops here where it will handle interrupts and
                    // passively update the screen->
  }
  while (1)
    ;
#else
...
void setup(void) { esp32_setup(); }
void loop(void) { esp32_loop(); }
#endif

#if defined(__PICO2__)
void pico2_setup(void);
void pico2_loop(void);
extern "C" void setup(void) { pico2_setup(); }
extern "C" void loop(void) { pico2_loop(); }
#endif
